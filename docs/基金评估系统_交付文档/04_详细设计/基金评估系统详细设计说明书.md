...
> **版本**：v1.0（对应 BRD v1.2 / 需求确认记录 DC-001~012 / 技术可行性评估 v1.2 / 技术规格说明书 v1.2 / 原型 v1.2）
...
### 1.1 编写目的
本文档承接《业务需求说明书 BRD v1.2》与《技术规格说明书 v1.2》，将 12 个业务模块（FR-D1~D6 + FR-01~48）的**功能逻辑、数据模型、接口契约、算法流程、安全控制、部署运维**落实到可直接施工的粒度。目标读者为开发者、测试、运维及后续维护者。
...
> Redis 为**可选增强层**，MVP 阶段可用 PostgreSQL + 应用内存缓存替代。若接入，建议缓存以下热数据：
...
- 若未来支持多用户，密码须 bcrypt 哈希，会话 token JWT 签名。
...
#### 3.14.4 接口设计
- `POST /api/admin/trigger-job`（手动触发，可选）。
...
#### ADR-004：Redis 可选缓存层
- **背景**：首页/榜单/字段解释等热数据可加速。
- **决策**：Redis 为可选项，MVP 可用 PostgreSQL + 内存缓存替代；生产建议接入。
- **后果**：降低 MVP 成本，保留扩展性。
...
*本文档由软件架构师基于 BRD v1.2、需求确认记录 DC-001~012、技术可行性评估 v1.2、技术规格说明书 v1.2 及原型 v1.2 产出，作为系统实现期的施工依据。*

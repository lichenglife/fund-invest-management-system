...
) PARTITION BY RANGE (trade_date);  -- 分区子策略(按年,pg_partman 自动建子分区)于 P1-22 部署期落地；MVP 阶段建普通表(数据量小性能可接受)，见 DEFERRED D1
...
CREATE TABLE scores (
  ...
  weights   JSONB NOT NULL,   -- {"ret":20,"risk":25,"perf":20,"scale":15,"manager":20}  -- 新五因子口径(E4/E5闭环)，权重见常量 SCORE_WEIGHTS
...
- 其余表（`managers`、`fund_dividends`、`discovery_entries`、`field_glossary`、`macro_indicators`、`market_sentiment`、`external_signal`、`sentiment_news`、`stock_financials`、`rag_documents`、`ai_weekly_reports`、`risk_assessments`、`alerts`、`valuation_signals`、`data_quality_log`、`notes`、`learning_paths`、`case_scenarios`、`case_replay_logs`、`behavior_assessments`、`data_collection_jobs`、`scheduler_jobs`）遵循同一约定；字段 DDL 在各模块落地时按 §3.x 数据库设计补充（当前 §3.x.4 仅列表明，字段定义随对应模块任务开工前补齐，如 P2-01 宏观前补 `macro_indicators`），迁移由 Alembic 统一管理。详见 DEFERRED D2。
...
```json
{ "code":"000001.OF", "weights":{"ret":20,"risk":25,"perf":20,"scale":15,"manager":20},
  "composite": 82.3,
...
#### 3.3.8.1 五因子综合评分合成（专家评审修订 E4 / E5）

> **因子重构（闭合 E4）**：原 `style`(风格适配) / `cost`(成本) 合并为更稳健的因子集，并修正 `scale` / `manager` 口径。新五因子：`ret`(收益) / `risk`(风险) / `perf`(业绩持续性) / `scale`(规模健康度) / `manager`(经理任期超额)。

- **因子与权重**：`ret`(20) / `risk`(25) / `perf`(20) / `scale`(15) / `manager`(20)，常量 `SCORE_WEIGHTS={"ret":20,"risk":25,"perf":20,"scale":15,"manager":20}`，可配置（ADR-002，仅在评估详情页调节）。
  - **降 ret 权重（E4 共线）**：`ret`/`risk`/`perf` 同源一条 NAV，高度共线；`ret` 由 30 降至 20、`perf` 吸收持续性信息，避免综合分信息冗余。
  - **scale → `scale_health`**：非"规模越大越好"。`aum∈[2亿,500亿]`=100；`<2亿` 触发清盘风险子分缓降；`>500亿` 因钝化缓降（E4）。
  - **manager → `manager_excess`**：= 任期区间基金收益 − 同类基准收益，表征**任期业绩**而非经理能力，报告须标注"任期超额、非能力归因"，含 β 与幸存者偏差（E4）。
  - **横截面分组维度（闭合 E5）**：百分位排名按**底层资产类别 `asset_class`**（equity/debt/money/alt/qdii）分组；货基收益/回撤恒近 0，排除出综合分排名，单独呈现，避免被规模主导。
  - **`max_drawdown` 符号约定（E5）**：子分函数约定 `max_drawdown` 返回**负值**（越负越差 → 子分越低），全局统一。
- **子分归一化**：每个因子原始指标先映射为 **0–100 子分 `s_k`**，默认采用**横截面百分位排名**（该基金在同 `asset_class` universe 中的百分位 × 100）；可选 z-score 标准化后经 Sigmoid 映射到 0–100。某因子原始值缺失则其 `s_k=None`，合成时剔除该因子权重。
- **综合分**：
  ```
...
#### 3.3.8.2 Brinson 业绩归因（仅 `BRINSON_SCOPE=["mixed","stock"]`，专家评审修订 E1 / E2）

- **输入（闭合 E1）**：使用披露**真实持仓权重**（前十大之和常 <1，严禁 normalize 至 1）。基金端未披露部分计入 **`OTHER_CASH` 残差桶**（`w_p,other = 1 − Σ 已披露`），基准端同样设 `OTHER_CASH` 残差桶（`w_b,other = 1 − Σ 基准已披露`）以匹配；层级收益 `R_p,i` / `R_b,i` 取各层实际收益。
- **三向分解（单期）**：
  ```
  ...
  ActiveReturn_t = Allocation + Selection + Interaction
  ```
- **多期链接（闭合 E2）**：各期主动收益**不可算术求和**（≠ 复利主动收益）。采用**几何链接 + Carino/Frongello 平滑因子 `k_t`**，将单期三效应折算到复合主动收益后再按 `k_t` 分配：
  ```
  k_t = (ln(1+AR_cum) − ln(1+AR_t)) / (AR_cum − AR_t)   # 平滑因子
  Effect_cum = Σ_t k_t · Effect_t                        # 配置/选券/交互分别几何链接
  ```
  报告须显式标注"几何链接、复利口径"；若仅展示单期则标注"单期口径"。
- **范围处理**：指数基（ETF / 被动）不显示 Brinson，改显跟踪误差 `TE` 与信息比率 `IR = α / TE`；债基不显示。持仓缺失（holdings 为空）则归因 `unavailable=True`。

#### 3.3.8.3 评估引擎 · 专家评审修订闭环（E1/E2/E4/E5/E7）

| 编号 | 严重度 | 原问题 | 本版修订 | 状态 |
|------|--------|--------|----------|------|
| E1 | 🔴 阻断 | 前十大 normalize 至权重和=1，虚增主动权重 | 用披露真实权重 + `OTHER_CASH` 残差桶（基金/基准各匹配），不 normalize | ✅ 已闭合 |
| E2 | 🟠 高 | 多期主动收益算术求和 ≠ 复利 | 几何链接 + Carino/Frongello 平滑因子 `k_t`；报告标注口径 | ✅ 已闭环 |
| E4 | 🟠 高 | scale 线性正向、manager=任职回报错误归因、ret/risk/perf 共线 | scale→`scale_health` 清盘门槛；manager→`manager_excess` 任期超额（标注非能力）；降 ret 权重至 20 | ✅ 已闭合 |
| E5 | 🟠 高 | 横截面按交易场所分组；回撤符号模糊；货基失真 | 改按 `asset_class` 分组；`max_drawdown` 约定负值；货基排除综合分排名 | ✅ 已闭合 |
| E7 | 🟠 高 | PEG/ERP 口径未定义（PE≤0、growth 越界、单一宽基 PE） | PEG 剔除 PE≤0 / growth 越界；ERP 拆大/小盘（见 §3.7）；代理口径相对高低提示 | ✅ 已闭合 |
| E13 | 🟢 低 | 风格箱缺债基/货基/QDII 不适用说明与回归窗口 | 风格箱限权益类；债/货/QDII/另类不显示；回归窗口固定 3y 并披露 | ✅ 已闭合 |
...
- 结果标注评分来源与截至时间。
- **NL「稳健/低风险」映射（闭合 E6，详见 TP-02 §3.3）**：规则兜底将"稳健|低风险"映射为 `risk<=15` 且 `type ∈ [bond, mixed]`，**排除 index/etf**（指数年波动~20%、回撤可达 30–40%，不属低风险）；`mixed` 进一步用 `equity_ratio<0.3` 字段细化，避免权益仓位过高的偏股混合被误判低风险。
...
- **NAV 口径与赎回费（闭合 E3，详见 TP-04）**：回放统一使用**后复权净值**（已含分红再投），不再叠加分红再投逻辑，杜绝与复权净值双重计；终值 `final_val` 须扣**赎回费**（默认长期持有档）后参与 IRR，新增 `gross_value` / `redeem_fee` 字段。
...
#### 3.6.6.1 诊断阈值与 risk_type 联动（专家评审修订 E8 / E9 / E12）

> **股债维阈值（闭合 E8）**：诊断"目标权益仓位"不再写死，改由**风险测评 `risk_type`** 推导：保守 20–40% / 稳健 40–60% / 进取 60–80%，并与 TP-05 股债中枢联动，形成"测评 → 中枢 → 诊断"一致链（详见 TP-03 §3.1）。
> **个基隐患阈值（闭合 E9 / E12）**：止损改"相对基准超额 < −15% **或** 最大回撤 > 30%"为红（含风格/基准，非绝对阈值）；止盈降为软提示（行为偏差，不自动）；费率预警阈值上调为**含托管综合费率 > 2.0%**（E12）。

---

### 3.6.7 回测并发与缓存设计（性能优化改造）

- 组合回测用 vectorbt 一次性向量化（§3.6.2）。
- **基准选择（闭合 E14，详见 TP-04）**：基准**默认取全收益指数**，并按组合底层 `asset_class` 自动选（权益→沪深300全收益 / 债券→中债综合财富 / QDII→标普500全收益），用户可覆盖；组合与基准均用后复权/全收益口径，分红处理一致。
...
#### 3.7.2.1 高位四维与股债中枢 · 专家评审修订（E10 / E11）

- **估值分位窗口固定（闭合 E11）**：所有"分位"指标固定为**近 10 年滚动分位**并披露，与 §3.3.8.1 百分表窗口一致；分位隐含均值回归假设但 A 股存在结构性断点，"偏高≠即将回调"，仅作相对贵贱提示，须与 ERP / 债券利差**交叉验证**（输出 `cross_check` 字段）。
- **股债中枢 ERP 拆分市值风格（闭合 E10）**：`equity_center` 分别计算**大盘 ERP** 与**小盘 ERP** 加权（7:3）得中枢，避免单一宽基口径误导；阈值 `(4%/40%)` 标注为初值，须经 `SENSITIVITY_TEST` 敏感性测试后写入 `macro_thresholds` 正式版（详见 TP-05 §3/§4）。

...
> **不搁置（业务/上线必做）**：A2 密钥轮换（安全，用户操作，公网上线前必做）、后台 /admin/jobs 与 /quality 端点（业务数据视图）、~~stylebox 风格箱算法（评估引擎业务）~~ ✅已实现（2026-08-07，见 CHANGELOG / `domain/stylebox.py`）、D6 adj_nav 真实复权（E3 业务正确性）、P1-23a 灰度上线本身。

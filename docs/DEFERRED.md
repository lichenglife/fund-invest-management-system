...
## D11 · 前端生产级改造：部署交付口径与前后端联调（CR-20260806-01 部分落地）

- **发现来源**：CR-20260806-01（前端生产级改造，C2，Approved 2026-08-06）原覆盖八维（颜色/组件/样式/大小/部署/联动/审美/合理性）。其中颜色/组件/样式/大小/响应式/可访问性已由 commit `7dee044` 实现并随 CR 关闭；**部署**与**联动**两维按用户拆分决策（"逐页收敛/响应式/部署/联调已拆成 4 个后续任务"）列为后续任务，未纳入本次关闭范围。
- **性质**：范围拆分（非缺陷、非回归）；纯前端表现层，不触动 DB / 接口契约 / E1–E14。
- **当前处理**：CR-20260806-01 在已实现范围内已 Closed（2026-08-07）；部署/联调留作后续，不静默丢弃。
- **影响**：生产交付须明确 `MOCK_MODE=0`、关闭 `runOnSave`、Docker Secrets 注入、静态资源/构建说明；前端须补 loading/empty/error 三态组件、Mock 角标贯穿、联调自检页。不影响已实现范围的合并与验收。
- **建议处置时点**：后续任务（原 #10 部署交付口径 + 前后端联调）单独排期；完成后可新开 CR 或并入发版验收，避免本 CR 静默扩大范围。
> 📌 **状态：延期项（范围拆分，非缺陷）**。依《需求变更管理规范》§7「部分落地须按 DEFERRED 登记」规则跟踪；CR-20260806-01 已实现范围已 Closed，部署/联调由本 D11 跟踪。
...
| D11 · 前端部署/联调(CR-20260806-01 部分落地) | 📌 延期（范围拆分） | 部署口径+三态组件+联调自检页，后续任务 | - |
